...
<div class="mb-5">
    ...
    <div class="float-end">
        <button type="submit" form="form-banner" class="btn btn-success "><i class="ri-save-3-line"></i></button>
        <button type="reset" form="form-banner" class="btn btn-secondary "><i class="bx bx-reset"></i></button>
        <a href="{{ route('admin.banner.index') }}" form="form-banner" class="btn btn-light border bg-white"><i class="ri-reply-line"></i></a>
    </div>
</div>
<div class="card">
    ...
    <form id="form-banner" method="post" action="{{ route('admin.banner.update', ['id'=> $banner->id]) }}" class="px-4" enctype="multipart/form-data">
        @if (count($errors) > 0)
            <div class="pt-4">
        ...
        <div class="tab-content" id="myTabContent">
            ...
            <div class="row mb-3 mt-3">
                ...
                <div class="col-sm-10">
                    <input id="image" class="form-control" type="file" name="image" value="{{ old('image', $banner->image) }}">
                </div>
            </div>
            @foreach($langs as $key => $lang)
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="title" name="title[{{ $lang['code'] }}]" required
                                   value="{{ old('title.'. $lang['code'], $banner->realTitle[$lang['code']]) }}">
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="info" class="col-sm-2 col-form-label required"> Инфо</label>
                        <div class="col-sm-10">
                            <textarea class="form-control" id="info" name="info[{{ $lang['code'] }}]" required rows="3">
                                {{ old('info.'. $lang['code'], $banner->realInfo[$lang['code']]) }}
                            </textarea>
                        </div>
                    </div>
                </div>

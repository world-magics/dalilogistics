...
<div class="card">
    <h5 class="card-header">Добавить новости</h5>
    ...
    <form id="form-news" method="post" action="{{ route('admin.news.store') }}" class="px-4" enctype="multipart/form-data">
        @if (count($errors) > 0)
            <div class="pt-4">
            ...
            @foreach($langs as $key => $lang)
                <div class="tab-pane fade {{ $key === 0 ? 'active show' : '' }}" id="{{ $lang['code'] }}" role="tabpanel" aria-labelledby="{{ $lang['code'] }}-tab">
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="title" name="title[{{ $lang['code'] }}]" required value="{{ old('title.'. $lang['code'], '') }}">
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="short_info" class="col-sm-2 col-form-label required">Инфо</label>
                        <div class="col-sm-10">
                            <textarea class="form-control" id="short_info" name="info[{{ $lang['code'] }}]" required rows="5">
                                {{ old('info.'. $lang['code'], '') }}
                            </textarea>
                        </div>
                    </div>
